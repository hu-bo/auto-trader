...
func (pm *PartitionManager) CheckAndPreCreate(ctx context.Context) error {
	if pm == nil {
	...
	if !pm.ShouldPreCreate() {
		logPartition.Debug().Msg("No need to pre-create next year")
		return nil
	}

	nextYear := currentYear + 1
	if _, err := pm.PreCreateNextYear(ctx); err != nil {
