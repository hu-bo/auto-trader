...
const (
	...
	defaultSubscribeBatchSize = 10
	defaultHistoryStartYear   = 2016
...
type HistorySyncService struct {
	...
	// 任务管理器
	taskManager *utils.SyncTaskManager
...
func (s *HistorySyncService) GetTaskManager() *utils.SyncTaskManager {
...
func (s *HistorySyncService) SyncHistoryData(ctx context.Context, exchangeName exchange.ExchangeName, symbols []exchange.SymbolInfo) error {
	...
	// 分批处理
	batches := utils.ChunkSlice(symbols, defaultSubscribeBatchSize)
...
func (s *HistorySyncService) findEarliestDataTime(ctx context.Context, client exchange.RESTClient, symbol string, tradeType exchange.TradeType) (*int64, error) {
	...
	startYear := defaultHistoryStartYear
...
func (s *HistorySyncService) GetSyncTaskStatus(exchangeName exchange.ExchangeName, symbol string, tradeType exchange.TradeType) *utils.SyncTask {
	if s.taskManager == nil {
		return nil
	}
