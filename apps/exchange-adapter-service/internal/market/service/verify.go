...
import (
	...
	"exchange-adapter-service/internal/market/storage"
	...
	for _, remote := range remoteCandles {
		...
		if local.Open != remote.Open {
			...
			result.Diffs = append(result.Diffs, CandleDiff{
				...
				DiffPercent: calcDiffPercent(local.Open, remote.Open),
			})
		}
		...
		if local.High != remote.High {
			...
			result.Diffs = append(result.Diffs, CandleDiff{
				...
				DiffPercent: calcDiffPercent(local.High, remote.High),
			})
		}
		...
		if local.Low != remote.Low {
			...
			result.Diffs = append(result.Diffs, CandleDiff{
				...
				DiffPercent: calcDiffPercent(local.Low, remote.Low),
			})
		}
		...
		if local.Close != remote.Close {
			...
			result.Diffs = append(result.Diffs, CandleDiff{
				...
				DiffPercent: calcDiffPercent(local.Close, remote.Close),
			})
		}

		// 对比 Volume (允许 5% 误差)
		volumeDiff := calcDiffPercent(local.Volume, remote.Volume)
		if volumeDiff > 5 {
			hasDiff = true
			result.Diffs = append(result.Diffs, CandleDiff{
				Timestamp:   remote.Timestamp,
...
// calcDiffPercent 计算差异百分比
func calcDiffPercent(local, remote float64) float64 {
	if remote == 0 {
		if local == 0 {
			return 0
		}
