...
import (
	...
	"exchange-adapter-service/internal/market/storage/db"
...
func (r *PostgresRepository) InitSchema(ctx context.Context) error {
	...
	// 启动 cron 定时预创建任务 (每天凌晨 1 点检查，提前 7 天创建下一年分表)
	if err := r.partition.StartCronPreCreate(""); err != nil {
		return fmt.Errorf("start cron pre-create: %w", err)
	}
