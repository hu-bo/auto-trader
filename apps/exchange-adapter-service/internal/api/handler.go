...
import (
	...
	"exchange-adapter-service/internal/market/service"
	"exchange-adapter-service/internal/market/storage"
	"exchange-adapter-service/internal/market/utils"
...
type GetSyncTasksData struct {
	Stats map[service.TaskStatus]int `json:"stats"`
	Tasks []*service.SyncTask        `json:"tasks,omitempty"`
}
...
func (h *Handler) GetSyncTasks(c echo.Context) error {
	...
	var tasks []*service.SyncTask

	if status != "" {
		tasks = taskManager.GetTasksByStatus(service.TaskStatus(status))
	} else {
