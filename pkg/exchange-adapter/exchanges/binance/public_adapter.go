...
func (c *MarketDataRESTClient) GetSymbols(ctx context.Context, tradeType marketdata.TradeType) ([]marketdata.SymbolInfo, error) {
	...
	case marketdata.Spot:
		var raw spotExchangeInfo
		if err := c.spot.Get(ctx, "/api/v3/exchangeInfo", nil, false, &raw); err != nil {
			return nil, err
		}

		out := make([]marketdata.SymbolInfo, 0, len(raw.Symbols))
		for _, s := range raw.Symbols {
	...
	case marketdata.Futures:
		var raw futuresExchangeInfo
		if err := c.futures.Get(ctx, "/fapi/v1/exchangeInfo", nil, false, &raw); err != nil {
			return nil, err
		}
...
func (c *MarketDataRESTClient) GetCandles(ctx context.Context, symbol string, tradeType marketdata.TradeType, period marketdata.Period, startTime, endTime int64, limit int) ([]marketdata.NormalizedCandle, error) {
	...
	for _, k := range rows {
		...
		openTime := core.Int64FromAny(k[0])
		openStr := core.StringFromAny(k[1])
		highStr := core.StringFromAny(k[2])
		lowStr := core.StringFromAny(k[3])
		closeStr := core.StringFromAny(k[4])
		volumeStr := core.StringFromAny(k[5])
		takerBuyVolumeStr := core.StringFromAny(k[9])
		...
		out = append(out, marketdata.NormalizedCandle{
			...
			Timestamp:    openTime,
			Open:         core.ParseFloat(openStr),
			High:         core.ParseFloat(highStr),
			Low:          core.ParseFloat(lowStr),
			Close:        core.ParseFloat(closeStr),
			Volume:       core.ParseFloat(volumeStr),
			BuyVolume:    core.ParseFloat(takerBuyVolumeStr),
